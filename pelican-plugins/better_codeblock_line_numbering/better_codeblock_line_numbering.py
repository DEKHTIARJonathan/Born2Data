...
def add_line_wrappers(data_passed_from_pelican):
    ...
    all_instances_of_pre_elements = re.findall('<pre>.*?</pre>',
                                               ...
                                               re.DOTALL)  # Use a regular expression to find every instance of '<pre>' followed by anything up to the first matching '</pre>'. re.DOTALL puts python's regular expression engine ('re') into a mode where a dot ('.') matches absolutely anything, including newline characters.

    if len(all_instances_of_pre_elements) > 0:
        # If the article/page HAS any <pre>...</pre> elements, go on. Otherwise,
        # don't (to do so would inadvertantly wipe out the output content for
        # that article/page).
        ...
        for pre_element_to_parse in all_instances_of_pre_elements:
            # Wrap each line of the <pre>...</pre> section with
            # <span class=code-line>...</span>, following
            # http://bililite.com/blog/2012/08/05/line-numbering-in-pre-elements/.
            # We'll use these to add line numbers using CSS later.
            # Note that below, '^' is the beginning of a string, '$' is the end
            # of a string, and '\n' is a newline.
            replacement_text_with_beginning_of_each_line_wrapped_in_span = re.sub(
                r'(<pre.*?>|\n(?!</pre>))', '\\1<span class="code-line">',
                pre_element_to_parse)  # The (?!...) here is a Negative
            # Lookahead (cf. http://www.regular-expressions.info/lookaround.html).
            # This full regular expression says "Give me all code snippets that
            # start with <pre ****> or start with a newline (\n), but NOT if the
            # newline is followed immediately with '</pre>'. Take whatever you
            # find, and replace it with what you found (\1) followed immediately
            # by '<span class="code-lines">'.
            # http://stackoverflow.com/a/14625628 explains why we need to escape
            # the backslash in the capture group reference (the '\1'). In short,
            # python will recognize it as "\x01" if it's not escaped.

            # Apply bug fix that would insert an empty line at the bottom.
            replacement_text_with_beginning_of_each_line_wrapped_in_span = \
                replacement_text_with_beginning_of_each_line_wrapped_in_span.replace(
                    "<pre><span class=\"code-line\"><span></span><code>",
                    "<pre><code><span class=\"code-line\">"
                )

            replacement_text_with_beginning_of_each_line_wrapped_in_span = \
                replacement_text_with_beginning_of_each_line_wrapped_in_span.replace(
                    "<span class=\"code-line\"></code></pre>",
                    "</code></pre>"
                )

            # # <pre><span class="code-line"><span></span><code>
            # raise Exception(replacement_text_with_beginning_of_each_line_wrapped_in_span)

            replacement_text_with_full_line_wrapped_in_span = re.sub(
                r'((?<!</pre>)$|(?<!</pre>)\n)', '</span>\\1',
                replacement_text_with_beginning_of_each_line_wrapped_in_span)
            # This regular expression says "Give me all code snippets that are
            # the end of a string or a newline (but not preceeded by "</pre>"
            # (this is a 'negative lookahead,' '(?<)'), and replace whatever you
            # found with '</span'> followed by whatever you found (\1).
